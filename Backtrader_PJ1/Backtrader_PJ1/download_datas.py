...
class SaveData:
    ...
    def __init__(self):
        ## 文件保存路径
        self.dir_base = properties.dir_base
        self.dir_chouma_base = os.path.join(self.dir_base, 'chouma_datas')
        self.dir_stock_codes_base = os.path.join(self.dir_base, 'stock_codes')
    ...
    def get_all_stocks(self):
        ...
        stock_codes_listed_filename = utils.save_out_filename(filehead='stocks_codes_listed_{formatted_date}',
                                                              file_type='txt')
    ...
    def get_chouma_datas(self):
        ...
        latest_stock_codes_file = os.path.join(self.dir_stock_codes_base,
                                               utils.get_latest_filename(self.dir_stock_codes_base))
        ...
        # dir_chouma_err_codes = os.path.join(self.dir_chouma_base, 'data_codes')
        # with open(r'E:\Quant\insight_A\chouma\index2\err_dict.txt', 'w') as f:
        #     f.write(str(err_dict))

        #################  记录无异常，能够找到筹码数据的codes   ###########################
        chouma_filename = utils.save_out_filename(filehead=f"chouma_{formatted_date}", file_type='xlsx')

        chouma_filename_dir = os.path.join(self.dir_chouma_base, chouma_filename)
        chouma_total_df.to_excel(chouma_filename_dir, float_format='%.0f', index=False)

        print("-------------  {} 可获得筹码数据的股票   共有{} 只：".format(formatted_date, len(list_res)))
        print("-------------  {} 不能获得筹码数据的股票 共有{}个元素：".format(formatted_date, len(err_dict)))
