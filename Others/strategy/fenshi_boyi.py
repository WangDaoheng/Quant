...
def Markowitz(total_codes, stock_returns):
    ...
    for i in range(number):
        ...
        random5 = np.random.random(5)                       ## 生成5个  0~1 的随机数
        ...
        random_p[i][:5] = random_weight             ## 权重
        random_p[i][5] = annual_return              ## 收益率
        random_p[i][6] = random_volatility          ## 标准差
...
class Fenshi_Strategy(bt.Strategy):

    def __init__(self):
        self.dataclose = self.datas[0].close
        self.rsi = bt.indicators.RSI_SMA(self.dataclose, period=14)
        self.lowest_low = self.dataclose
        self.lowest_rsi = self.rsi
    ...
    def next(self):
        if not self.buy_signal_triggered:
            if self.data.datetime.time() >= datetime.time(9, 45) and self.data.datetime.time() <= datetime.time(10, 45):
                if self.dataclose[0] < self.lowest_low[0]:
                    self.lowest_low[0] = self.dataclose[0]
                if self.rsi[0] > self.lowest_rsi[0] and self.dataclose[0] == self.lowest_low[0]:
